...
template<class TEntity>
void DbSet<TEntity>::set_options(DbContextOptionsBuilder* options)
{
	this->options_ = options;
}

template<typename TEntity>
bool DbSet<TEntity>::add(TEntity& entity)
{
	return add_range(std::list<TEntity> {entity});
}

template<class TEntity>
bool DbSet<TEntity>::add_range(std::list<TEntity>& entities)
{
...
template<typename TEntity>
bool DbSet<TEntity>::create()
{
}

template<typename TEntity>
bool DbSet<TEntity>::remove()
{
	return true;
}

template<class TEntity>
TEntity DbSet<TEntity>::where(std::function<bool()> expression)
{
	return TEntity();
}

template<class TEntity>
TEntity DbSet<TEntity>::single(int id)
{
	const rttr::type instance = rttr::type::get<TEntity>();

	std::stringstream expression;

	expression << Simbols::LEFT << instance.get_name().to_string();
	expression << Simbols::RIGHT << Simbols::POINT;
	expression << "Id" <<  Simbols::EQUALLY << std::to_string(id);

	return single(expression.str());
}

template<class TEntity>
TEntity DbSet<TEntity>::single(const STRING expression)
{
	STRING table_name;
...
template<class TEntity>
std::vector<TEntity> DbSet<TEntity>::all()
{
	STRING table_name;
...
template<class TEntity>
bool DbSet<TEntity>::update(TEntity& entity)
{
	return false;
}

template<class TEntity>
std::vector<TEntity> DbSet<TEntity>::executeQueryWithResultIEntity(const STRING query)
{
...
template<class TEntity>
void DbSet<TEntity>::executeQuery(const STRING query)
{
	switch (options_->get_provider()){
	case SqlProvider::MS_SQL:
...
template<class TEntity>
PROPERTIES_SET DbSet<TEntity>::getEntityProperties(STRING& table_name)
{
	const rttr::type instance = rttr::type::get<TEntity>();
...
template<class TEntity>
PROPERTIES_MAP DbSet<TEntity>::getAddedValues(const rttr::array_range<rttr::property>& properties, const std::list<TEntity>& entities)
{
	PROPERTIES_MAP fields;

	if (entities.empty() || properties.empty()) return fields;
	...
	for (TEntity entity : entities) 
	{
		...
		columns << Simbols::LEFTROUND;
		values << Simbols::LEFTROUND;
		...
		for (var& prop : properties) 
		{
			...
			columns << Simbols::LEFT << prop.get_name().to_string() << Simbols::RIGHT;
			...
			if (num_properties > 0) {
				values << Simbols::COMMA << Simbols::SPACE;
				columns << Simbols::COMMA << Simbols::SPACE;
			}
		}
		...
		fields.insert(std::pair(columns.str(), values.str()));
	}
